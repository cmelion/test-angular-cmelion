...
describe('Model: MyTestModel', function () {

    var MyTestModel, $httpBackend, $rootScope, APIBaseUrl, MyTestRepository, collectionUrl = 'my-tests';

    beforeEach(function () {

        MyTestRepository = jasmine.createSpy('MyTestRepository');
        MyTestRepository.attach = jasmine.createSpy('MyTestRepository.attach');

        module('testAngularCmelion', function ($provide) {
            $provide.value('MyTestRepository', MyTestRepository);
        });

        inject(function (_MyTestModel_, _$httpBackend_, _$rootScope_, _APIBaseUrl_) {
            MyTestModel = _MyTestModel_;
    ...
    it('should have the url property set', function() {
        expect(MyTestModel.$settings.url).toBe(APIBaseUrl + collectionUrl);
    });
    ...
    describe('$save', function () {
        it('should PUT its data on $save when it has an ID (update existing)', function() {
            ...
            var model = new MyTestModel({title: 'New title', id: 5});
        ...
        it('should POST its data on $save if does not have an ID (new)', function() {
            ...
            var model = new MyTestModel({title: 'New title'});
        ...
        it('should attach itself to the Repository on save', function() {
            $httpBackend.expectPUT( APIBaseUrl + collectionUrl + '/5', {title:'New title', id:5}).respond(200, {id: 5, title:'New title from server'});
            var model = new MyTestModel({title: 'New title', id: 5});
            expect(MyTestRepository.attach).not.toHaveBeenCalled();
            var promise = model.$save();
            $httpBackend.flush();
            expect(MyTestRepository.attach).toHaveBeenCalledWith(model);
        });
    });
    ...
    describe('$set', function () {
        it('should load instance and override with new data', function() {
            ...
            expect(model instanceof MyTestModel).toBeTruthy();
        });
    ...
    describe('$isDirty', function () {
        it('should return false if object is not changed since last save or delete ', function() {
            var model = new MyTestModel({id:1});
            expect(model.$isDirty).toBeFalsy();
        });

        it('should not be dirty initially', function() {
            var model = new MyTestModel({id:5});
            expect(model.$isDirty).toBeFalsy();
            $rootScope.$digest();
            expect(model.$isDirty).toBeFalsy();
        });
